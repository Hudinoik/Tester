...
const jwt=require("jsonwebtoken");const cors=require("cors");const path=require("path");
...
app.post("/users",auth,allow("superadmin"),async(req,res)=>{
  ...
  const h=await bcrypt.hash(password,10); db.run("INSERT INTO users(email,password,role) VALUES(?,?,?)",[email,h,role],function(err){res.status(err?400:200).json(err?{error:err.message}:{id:this.lastID});});
});
app.put("/users/:id",auth,allow("superadmin"),async(req,res)=>{
  const id=parseInt(req.params.id,10); const{role,password}=req.body; const sets=[],p=[];
...
app.put("/users/:id/shuls",auth,allow("superadmin"),(req,res)=>{
  ...
  db.serialize(()=>{ db.run("DELETE FROM user_shuls WHERE userId=?",[id],e=>{
...
app.post("/shuls",auth,allow("superadmin","admin"),async(req,res)=>{
  ...
  db.run("INSERT INTO shuls(name,address,area,lat,lon) VALUES(?,?,?,?,?)",[name,address||"",area||"",lat,lon],function(err){
    res.status(err?400:200).json(err?{error:err.message}:{id:this.lastID,lat,lon});
  });
});
